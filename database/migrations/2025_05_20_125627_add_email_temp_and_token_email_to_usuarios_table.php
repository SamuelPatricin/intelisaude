...
return new class extends Migration { // cria classe que herda o migration
    ...
    public function up()
    {
        Schema::table('usuarios', function (Blueprint $table) { // edita tabela
            $table->string('email_temp')->nullable(); // cria coluna (possivel nula)
            $table->string('token_email')->nullable();  // cria coluna (possivel nula)
        });
    }
    ...
    {
        Schema::table('usuarios', function (Blueprint $table) { // edita tabela
            $table->dropColumn(['email_temp', 'token_email']); // remove o email_temp e o token
        });
    }
};
