...
class AuthController extends Controller
{
    public function register(Request $request)
    {
        ...
        if ($usuarioExistente) {
            if ($usuarioExistente->verificado) {
                ...
                // Reenvia novo código para usuário ainda não verificado
                $codigo = strtoupper(substr(bin2hex(random_bytes(3)), 0, 6));
                $usuarioExistente->codigo_verificacao = $codigo;
                $usuarioExistente->senha = bcrypt($request->senha); // Atualiza a senha caso o usuário tente corrigir
    ...
    private function enviarEmail($nome, $email, $codigo)
    {
        ...
        try {
            ...
            $mail->Body = "Olá, <b>$nome</b>. <br><br>
                        ...
                        <br><br>Este código é válido por 15 minutos, além de ser pessoal, intransferível e não deve ser compartilhado com terceiros.
    ...
    public function verificarCodigo(Request $request)
    {
        ...
        // converte o código recebido para maiúsculo para comparar
        ...
        if ($usuario && $codigoDigitado === $usuario->codigo_verificacao) {
            ...
            Auth::login($usuario); // Faz login automático após verificação
