const inputNome = document.getElementById('inputNome');
const botaoEditar = document.getElementById('editarNome');
const token = document.getElementById('token_csrf').value;
const mensagem = document.getElementById('mensagemSucesso');

let editando = false;

botaoEditar.addEventListener('click', () => {
    if (!editando) {
        inputNome.removeAttribute('readonly');
        inputNome.focus();
        editando = true;

        botaoEditar.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                <path d="M13.485 1.929a.75.75 0 0 1 1.06 1.06L6.78 
                         10.753l-3.72-3.72a.75.75 0 1 1 
                         1.06-1.06l2.69 2.69 6.674-6.674z"/>
            </svg>`;
    } else {
        // Enviar nome atualizado para backend
        fetch('/atualizar-nome', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': token
            },
            body: JSON.stringify({
                nome: inputNome.value
            })
        })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'ok') {
                inputNome.setAttribute('readonly', true);
                editando = false;

                botaoEditar.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                        fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                        <path d="M15.502 1.94a.5.5 0 0 1 0 
                                 .706L14.459 3.69l-2-2L13.502.646a.5.5 
                                 0 0 1 .707 0l1.293 1.293zm-1.75 
                                 2.456-2-2L4.939 
                                 9.21a.5.5 0 0 0-.121.196l-.805 
                                 2.414a.25.25 0 0 0 
                                 .316.316l2.414-.805a.5.5 0 0 
                                 0 .196-.12l6.813-6.814z"/>
                        <path fill-rule="evenodd"
                            d="M1 13.5A1.5 1.5 0 0 0 
                               2.5 15h11a1.5 1.5 0 0 
                               0 1.5-1.5v-6a.5.5 0 0 
                               0-1 0v6a.5.5 0 0 
                               1-.5.5h-11a.5.5 0 0 
                               1-.5-.5v-11a.5.5 0 0 
                               1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 
                               1.5 0 0 0 1 2.5z"/>
                    </svg>`;

                // Mostrar mensagem
                mensagem.style.display = 'block';

                // Esconder depois de 2,5 segundos
                setTimeout(() => {
                    mensagem.style.display = 'none';
                }, 2500);

            } else {
                alert('Erro ao atualizar nome.');
            }
        })
        .catch(error => {
            console.error('Erro:', error);
            alert('Erro ao atualizar nome.');
        });
    }
});
